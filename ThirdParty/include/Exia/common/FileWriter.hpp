...
namespace eio
{
	...
	public:
		...
		size_t Write(const wchar_t* buffer, size_t size)
		{
			...
			{
				std::string tmp = mSci.Wchar2Char(buffer, size);
				mAFile << tmp;
			}
			return 0;
		}
	...
	private:
		void InitFileInternal()
		{
...
#ifdef _WIN32				
				mUFile.imbue(std::locale(std::locale::classic(), new ::std::codecvt_utf8<wchar_t>));
#else
			...
			else
			{
				...
				mUFile.open(mFile);
#ifdef _WIN32
				mUFile.imbue(std::locale(std::locale::classic(), new std::codecvt_utf16<wchar_t>));
#else
				mUFile.imbue(std::locale("en_US.UTF-8"));
#endif
			}
	...
	public:
		...
		size_t Write(const char* buffer, size_t size)
		{
			if (mIsUFile)
			{
				std::wstring tmp = mSci.Char2Wchar(buffer, size);
				mUFile << tmp;
			}
			else
			{
