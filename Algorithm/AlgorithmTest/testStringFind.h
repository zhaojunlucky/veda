...
void testStringRFind(const char* str, size_t size, const char* pattern, size_t patSize)
{
	cout << str << ":" << pattern << "  " << rfindSubStr<char>(str, size, pattern, patSize) << endl;
}
void testStringRFind()
{
	...
	string str1 = "how old are you, this is a test for find a substring in a string";
	testStringRFind(str1.c_str(), str1.size(), "how", 3);
	testStringRFind(str1.c_str(), str1.size(), "old", 3);
	testStringRFind(str1.c_str(), str1.size(), " ", 1);
	testStringRFind(str1.c_str(), str1.size(), " are", 4);
	testStringRFind(str1.c_str(), str1.size(), "a", 1);
	testStringRFind(str1.c_str(), str1.size(), "substring", 9);
	testStringRFind(str1.c_str(), str1.size(), "string", 6);
	string sub1 = "find a substring in a string";
	testStringRFind(str1.c_str(), str1.size(), sub1.c_str(), sub1.size());
	testStringRFind(str1.c_str(), str1.size(), "z", 1);
}

void testStringRFindAll(const char* str, size_t size, const char* pattern, size_t patSize)
{
	SubStringIndexesPtr indexes = rfindAllSubStr<char>(str, size, pattern, patSize);
	cout << str << ":" << pattern << ", indexes:";
	if (indexes->getLength() == 0)
	{
		cout << "not found" << endl;
...
void testStringRFindAll()
{
	string str1 = "how old are you, this is a test for find a substring in a string, yes you";
	testStringRFindAll(str1.c_str(), str1.size(), "z", 1);
	testStringRFindAll(str1.c_str(), str1.size(), "a", 1);
	testStringRFindAll(str1.c_str(), str1.size(), "you", 3);
}