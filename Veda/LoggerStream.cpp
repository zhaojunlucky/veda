...
namespace veda
{
	...
	LoggerStream::LoggerStream(const wchar_t* file, int line, Severity severity, bool writeToFile, LoggerWriter* loggerWriter)
		: mSeverity(severity), mWriteToFile(true), mLoggerWriter(loggerWriter)
	{
		mFile.reset(new String(file));
		mMessage.reset(new String);
	...
	{
		*this = std::move(loggerStream);
	}

	LoggerStream& LoggerStream::operator = (LoggerStream&& loggerStream)
	{
		this->mFile.swap(loggerStream.mFile);
		this->mLine = loggerStream.mLine;
		this->mLoggerWriter = loggerStream.mLoggerWriter;
		this->mMessage.swap(loggerStream.mMessage);
	...
	LoggerStream::~LoggerStream()
	{
		if (mMessage.get() != 0)
		{
			if (mWriteToFile && mLoggerWriter)
			{
				mLoggerWriter->write(mMessage->c_str(), mMessage->getSize());
			}
